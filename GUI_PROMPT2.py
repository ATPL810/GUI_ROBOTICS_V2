...
class MainWindow:
    def __init__(self):
        ...
        self.root.geometry("1300x700")  # Increased width for larger camera
    ...
    def init_ui(self):
        # Configure grid weights - giving more space to camera (3:1 ratio)
        self.root.grid_columnconfigure(0, weight=3)  # Camera area (increased from 2 to 3)
        ...
        left_frame.grid_rowconfigure(0, weight=4)  # Increased camera weight (from 3 to 4)
        left_frame.grid_rowconfigure(1, weight=1)  # Controls
        left_frame.grid_rowconfigure(2, weight=2)  # Tools list
        
        # Camera display - ENLARGED
        ...
        # Control buttons grid (3 columns)
        ...
        self.home_button.grid(row=0, column=1, padx=5, pady=5, sticky="ew")
        
        self.exit_button = tk.Button(control_frame, text="🚪 Exit System", 
                                    font=("Arial", 11, "bold"),
                                    bg="#f38ba8", fg="#1e1e2e",
        ...
        self.fetch_prompt_button.grid(row=1, column=0, columnspan=3, padx=5, pady=5, sticky="ew")
        ...
        # Progress bar
        self.progress_bar = ttk.Progressbar(control_frame, length=500, mode='determinate')
    ...
    def update_camera(self):
        ...
        if self.current_frame is not None:
            try:
                ...
                # Resize to fit label - USING LARGER DISPLAY
                h, w, _ = rgb_image.shape
                max_h = self.camera_label.winfo_height() or 600  # Increased from 480
                max_w = self.camera_label.winfo_width() or 800   # Increased from 640
    ...
    def on_closing(self):
        ...
        response = messagebox.askyesno("Exit", "Are you sure you want to exit the Garage Assistant Pro?")
        if response:
            self.log("🚪 Exiting Garage Assistant Pro...", "system")
            ...
            try:
                if self.arm_controller:
                    self.arm_controller.go_to_home()
                    time.sleep(1)  # Give time to move to home
            except:
                pass
